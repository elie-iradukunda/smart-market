...
export default function ControllerTopNav({ onMenuClick }: ControllerTopNavProps) {
  ...
  return (
    <header className="bg-gradient-to-r from-amber-900 via-orange-900 to-amber-900 text-white shadow-lg sticky top-0 z-30 backdrop-blur-sm border-b border-amber-800/50">
      <div className="mx-auto max-w-7xl px-3 sm:px-4 lg:px-6 h-14 sm:h-16 flex items-center justify-between gap-3 sm:gap-6">
        ...
        <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-shrink-0">
          <button
            type="button"
            className="md:hidden p-2 rounded-lg text-amber-200 hover:text-white hover:bg-white/10 transition-colors"
            onClick={onMenuClick}
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            <div className="relative">
              ...
              <span className="relative inline-flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 text-xs sm:text-sm font-bold text-white shadow-lg ring-2 ring-white/20">
                CO
              </span>
            </div>
            <div className="leading-tight hidden sm:block">
              <p className="text-[9px] sm:text-[10px] uppercase tracking-[0.2em] text-amber-200/90 font-medium">Role</p>
              <p className="text-xs sm:text-sm font-bold text-white">Controller</p>
            </div>
          </div>
        </div>
