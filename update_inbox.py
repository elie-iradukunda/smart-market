...
# Read the file
with open(r'c:\Users\user\Desktop\LanariMarket\smart-market\frontend\src\pages\Communications\InboxPage.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# Define the replacement
old_pattern = r'(\s+\) : \(\s+<div className="space-y-3">\s+\{orders\.map\(\(order: any\) => \(\s+<div\s+key=\{order\.id\}[\s\S]+?</div>\s+\)\)\}\s+</div>)'
...
# Write back
with open(r'c:\Users\user\Desktop\LanariMarket\smart-market\frontend\src\pages\Communications\InboxPage.tsx', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("WhatsApp functionality added to InboxPage successfully!")
